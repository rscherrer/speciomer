...
#'
#' @param root,root_init,add_rect Arguments to pass to \code{plot_eco_trait_zoomed} and
...
plot_eco_zoomed <- function(root, root_init, add_rect = FALSE) {

  # Plot zoomed trait densities
  plot_traits <- plot_eco_trait_zoomed(root, root_init, add_rect)

  # Plot zoomed average mutational effects through time
  plot_alphas <- plot_eco_alpha_zoomed(root, root_init, add_rect) + rm_strips()
